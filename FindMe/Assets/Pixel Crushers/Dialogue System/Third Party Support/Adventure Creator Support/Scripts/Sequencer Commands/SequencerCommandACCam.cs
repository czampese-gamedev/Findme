...
namespace PixelCrushers.DialogueSystem.SequencerCommands
{
    ...
    public class SequencerCommandACCam : SequencerCommand
    {
        ...
        {
            foreach (var cam in GameObjectUtility.FindObjectsByType<_Camera>())
            {
                if (DoesGameObjectPathMatch(cam.transform, cameraName))
                {
        ...
        private bool DoesGameObjectPathMatch(Transform t, string requiredPath)
        {
            if (t == null) return false;
            if (requiredPath.Contains("/"))
            {
                var fullPath = GetPath(t);
