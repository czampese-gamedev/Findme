...
namespace AC
{
	...
	public class ActionDialogueSystemConversation : Action
	{
        ...
        public override string Title { get { return "Dialogue System Conversation"; } }
        public override string Description { get { return "Starts a Dialogue System conversation. Leave Actor blank to default to the player."; } }
        ...
        override public float Run ()
		{
			if (!isRunning)
			{
                ...
                {
                    conversationActor = KickStarter.player.transform;
                }

				if (conversationActor == null)
                {
					GameObject actorGameObject = GameObject.FindGameObjectWithTag("Player");
					if (actorGameObject != null) conversationActor = actorGameObject.transform;
				}

                // Start the conversation:
                if (specifyEntryID)
                {
                    DialogueManager.StartConversation(conversation, conversationActor, conversant, entryID);
                }
                else
                {
                    DialogueManager.StartConversation(conversation, conversationActor, conversant);
                }
        ...
        private ActionConversationPicker conversationPicker = null;
        private ActionDialogueEntryPicker entryPicker = null;
		private DialogueDatabase selectedDatabase = null;
        ...
        override public void ShowGUI(List<ActionParameter> parameters)
        {
            ...
            if (actionMode != ActionMode.StopConversation)
            {
                ...
                actorIsPlayer = EditorGUILayout.Toggle(new GUIContent("Actor is player?", "Use current player as conversation actor."), actorIsPlayer);
                if (!actorIsPlayer)
                {
